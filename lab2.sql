...
ON 
    CAST(Н_ОЦЕНКИ.КОД AS TEXT) = Н_ВЕДОМОСТИ.ОЦЕНКА
WHERE 
    CAST(Н_ОЦЕНКИ.КОД AS INTEGER) < 5 
...
LEFT JOIN 
    Н_СЕССИЯ 
ON 
    Н_ВЕДОМОСТИ.СЭС_ИД = Н_СЕССИЯ.СЭС_ИД 
WHERE 
